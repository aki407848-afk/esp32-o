...
// ИСПРАВЛЕНО: надежное сканирование через esp_wifi API
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// Сканирование сетей (НАДЕЖНЫЙ МЕТОД через esp_wifi)
// ═══════════════════════════════════════════════════════════════
void scanNetworks() {
    ...
    // Полная перезагрузка WiFi стека
    WiFi.disconnect(true);
    esp_wifi_stop();
    delay(500);
    
    // Инициализация WiFi конфигурации
    wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
    esp_err_t err = esp_wifi_init(&cfg);
    
    if (err != ESP_OK && err != ESP_ERR_WIFI_INIT_STATE) {
        Serial.printf("❌ Ошибка инициализации WiFi: %d\n", err);
        return;
    }
    ...
    if (err != ESP_OK) {
        Serial.printf("❌ Ошибка установки режима: %d\n", err);
        return;
    }
    ...
    if (err != ESP_OK && err != ESP_ERR_WIFI_CONN) {
        Serial.printf("❌ Ошибка запуска WiFi: %d\n", err);
        return;
    }
    ...
    for (int i = 0; i < networkCount; i++) {
        networks[i].ssid = String((char*)ap_records[i].ssid);
